oo.PhotoViewer = function(options) {
  ...
  guiLoader.addImages(hui.ui.getContext()+'hui/gfx/progress/spinner_grey_32.gif');
  guiLoader.addImages(hui.ui.getContext()+'hui/gfx/progress/spinner_grey_24.gif');
  guiLoader.load();
}
  ...
  _redraw : function() {
    if (this._checkSize()) {
      ...
      for (var i=0; i < this.images.length; i++) {
        ...
        urls.push('/service/image/id'+this.images[i].id+'width'+this.size.width+'height'+this.size.height+'_inherit.jpg');
      };
  ...
  _build : function() {
    ...
    for (var i=0; i < this.images.length; i++) {
      ...
      var t = hui.build('a',{
        ...
        parent : this.thumbs,
        style : {
          backgroundImage : 'url(/service/image/id'+image.id+'width100height100sharpen1.0cropped_inherit.jpg'+')'
        }
      });
