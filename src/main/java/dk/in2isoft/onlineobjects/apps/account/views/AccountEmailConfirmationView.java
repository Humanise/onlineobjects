...
public class AccountEmailConfirmationView extends AbstractManagedBean {
	...
	public void before(Request request) throws Exception {
		...
		try {
			Pair<EmailAddress, String> mailToName = memberService.findEmailByConfirmationKey(key);
			memberService.markCondifirmed(mailToName.getKey(), request.getSession());
